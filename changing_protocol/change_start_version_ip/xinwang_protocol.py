...
from change_start_version_ip.normal_query import get_setting, get_access_token, start_rechage_send_tempalte, invalid_dealer_order, \
...
def xinwang_protocol(data, conn, addr):
    ...
    if byte_len == len(data):
        ...
        if cmd == '20':  #握手
            ...
            cmd = f"""
            update wxapp_pod_pile set lastip='{addr[0]}',pileversion='{pileversion}',iccid='{iccid}',
            ...
            conn.send(repsend)
            sob_handle = sob.sql_open(db_config)
            cmd = f"""update wxapp_pod_pile set lastip='{addr[0]}',isonline=1,snum='{serialnum}',type=1,update_time='{timer.get_now()}'
                    where serialnum='{serialnum}'
                    """
            sob.update_mysql_record(sob_handle, cmd)
            ...
            sob_handle = sob.sql_open(db_config)
            cmd = f"select * from wxapp_pod_pile where lastip='{addr[0]}'"
