...
from change_start_version_ip.normal_query import over_recharge, shua_card_rechage, get_setting, get_access_token, \
...
def bolai_protocol(data, conn, addr):
    ...
    if cklen == len(data):
        ...
        if cmd == '0000':
            ...
            cmd = f"""update wxapp_pod_pile set xhqd='{xhqd}',pileversion="{sob.escape(version)}",lastip='{addr[0]}',update_time='{timer.get_now()}',isonline=1 where gateway_id='{onlytag}'"""
            prolog.info(cmd)
            ...
            if childcmd == '1c':
                ...
                if pod_pile:
                    pod_pile = pod_pile[0]
                    cmd = f"update wxapp_pod_pile set pileport={int(num)},lastip='{addr[0]}' where id={pod_pile['id']}"
                    sob.update_mysql_record(sob_handle,cmd)
            ...
            if value_cmd == '1017':
                ...
                cmd = f"update wxapp_pod_pile set lastip='{addr[0]}',pileversion='{version}' where gateway_id='{onlytag}' and serialnum='{node_number}'"
                sob.update_mysql_record(sob_handle, cmd)
                sob.sql_close(sob_handle)
                # 平台回复
                repheard = 'fcff'
            ...
            if resp:
                ...
                if value_cmd == '1001':
                    ...
                    repdatetime = nowtime.strftime("%Y%m%d%H%M%S")
                    sob_handle = sob.sql_open(db_config)
                    cmd = f"""update wxapp_pod_pile set xhqd='{xhqd}',pileversion="{value_version}",iccid='{value_iccid}',lastip='{addr[0]}',update_time='{timer.get_now()}',isonline=1 where snum='{value_mac}'"""
                    sob.update_mysql_record(sob_handle, cmd)
                    sob.sql_close(sob_handle)
                    redatalist = [
                        {
                    ...
                    if pod_pile:
                        ...
                        for item in resp[4:]:
                            ...
                            try:
                                ...
                                prolog.error(f'柏莱电流数据入库失败,error:{e}')
                    cmd = f"update wxapp_pod_pile set pileport={count},lastip='{addr[0]}' where snum='{value_mac}'"
                    sob.update_mysql_record(sob_handle,cmd)
                    sob.sql_close(sob_handle)
                    # 平台回复
                    repheard = 'fcff'
                    ...
                    cmd = f"select * from wxapp_pod_pile where snum='{value_mac}'"
                    pod_pile = sob.select_mysql_record(sob_handle, cmd)
                    repcode,lastrowid = shua_card_rechage(pod_pile, value_card, value_pile, value_mac)
                    if repcode == '01':
                        # 平台回复
                        repheard = 'fcff'
                        duration = dec2hex(720)
